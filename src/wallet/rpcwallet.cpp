...
UniValue sendfromaddress(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() < 3 || request.params.size() > 8)
        throw std::runtime_error(
            ...
            "\nSend an amount from a specific address to a given address. All SATOX change will get sent back to the from_address\n"
            + HelpRequiringPassphrase(pwallet) +
            "\nArguments:\n"
...
{
    throw std::runtime_error("bumpfee has been deprecated on the SATOX Wallet.");
