...
static const std::regex SATOXCOIN_NAMES("^RVN$|^RAVEN$|^RAVENCOIN$|^#RVN$|^#RAVEN$|^#RAVENCOIN$|^SATOX$|^SATOX$|^SATOXCOIN$|^SATOXCOINCOIN$|^#SATOX$|^#SATOXCOIN$|^#SATOXCOINCOIN$");
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(SATOX_N); 
    vchMessage.push_back(SATOX_E);
    vchMessage.push_back(SATOX_X); 
    vchMessage.push_back(SATOX_Q); 
...
{
    // Must contain at least 3 outpoints (SATOX burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct SATOX Amount
...
bool CreateAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewAsset> assets, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    // Check to make sure the wallet has the SATOX required by the burnAmount
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::vector<std::pair<CNullAssetTxData, std::string> >* nullAssetTxData, std::vector<CNullAssetTxData>* nullGlobalRestrictionData)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any SATOX, transfering an asset requires a network fee"));
        return false;
    }
    ...
    // If assetTxData is not nullptr, the user wants to add some OP_SATOX_ASSET data transactions into the transaction
    if (nullAssetTxData) {
        std::string strError = "";
        int nAddTagCount = 0;
    ...
    // nullGlobalRestiotionData, the user wants to add OP_SATOX_ASSET OP_SATOX_ASSET OP_SATOX_ASSETS data transaction to the transaction
