...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for SATOX
    ...
    if (p_rewardSnapshot.strDistributionAsset != "SATOX") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is SATOX with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using SATOX differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "SATOX") {
        // Check amount
