...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs, bool fMempoolCheck, bool fBlockCheck)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        /** SATOXCOIN START */
        // Find and handle all new OP_SATOX_ASSET null data transactions
    ...
    if (tx.IsNewAsset()) {
        ...
        // above transaction types.  Also fail if it contains OP_SATOX_ASSET opcode but wasn't a valid script.
        for (auto out : tx.vout) {
            int nType;
            bool _isOwner;
            if (out.scriptPubKey.IsAssetScript(nType, _isOwner)) {
                ...
                if (out.scriptPubKey.Find(OP_SATOX_ASSET)) {
