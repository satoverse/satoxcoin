...
bool parseSatoxcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!SatoxcoinUnits::parse(SatoxcoinUnits::SATOX, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
QString formatSatoxcoinURI(const SendCoinsRecipient &info)
{
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1").arg(SatoxcoinUnits::format(SatoxcoinUnits::SATOX, info.amount, false, SatoxcoinUnits::separatorNever));
        paramCount++;
    }
...
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", SATOXCOIN_CONF_FILENAME));
