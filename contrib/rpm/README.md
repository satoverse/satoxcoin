...
The RPM spec file provided here is for Satoxcoin-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://satoverse.io/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Satoxcoin git repository](https://github.com/satoverse/Satoxcoin) but are not part of
...
Currently only `Source10` is used, for the example `satoxcoin.conf` file.
...
in the satoxcoin git repository. At present only an SVG version of the Satoxcoin
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the satoxcoin
...
needed when 0.12.1 is released, a proper fix is already in the Satoxcoin git
master branch.
...
idea to build Satoxcoin Core against a static version of that library compiled
...
The `satoxcoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `satoxcoin-server` package. At this point, most RPM based
...
The `satoxcoin-server` package should have SELinux support. How to properly do
...
Satoxcoin-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/satoxcoin/). He has been
packaging Satoxcoin for Fedora at least since 2012.
...
4. I renamed the `satoxcoin` package that contains the Qt GUI to `satoxcoin-core` as
that appears to be how the general population refers to it, in contrast to
`satoxcoin-xt` or `satoxcoin-classic`. I wanted to make sure the general population
